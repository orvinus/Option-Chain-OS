...
/**
 * Fixed username+password gate for the /hidden dashboard. Credentials are verified
 * by the backend (against HIDDEN_USER / HIDDEN_PASSWORD in .env) — never checked in
 * the browser. A successful sign-in also starts the broker feed server-side, so this
 * can block up to ~45s; the button shows a loading state until it resolves.
 */
export function HiddenLogin({ onAuthenticated }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await api.hiddenLogin({ username: username.trim(), password });
      onAuthenticated?.();
    } catch (err) {
      setError(String(err));
    } finally {
