...
export function Dashboard() {
  ...
  // Fixed-credential gate for /hidden. Independent of the (global) broker
  ...
  const handleAuthenticated = useCallback(() => {
    ...
    // Optimistic — safe because /hidden-login blocks until the broker session is
    // live, so the next health poll confirms it (no empty-body flash).
  ...
  return (
    <div className="min-h-screen w-full max-w-[1500px] mx-auto px-4 md:px-6 py-3">
      ...
      <main className="flex flex-col gap-4">
        {!gateUnlocked ? (
          <HiddenLogin onAuthenticated={handleAuthenticated} />
        ) : (
          <>
            {/* ── Controls row ──────────────────────────────────── */}
